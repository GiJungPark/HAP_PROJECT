...
- 헤더에 토큰을 삽입하여 호출해야 한다. (Body에 별도의 값을 기입할 필요없다.)
...
- Body에 이메일, 인증 번호를 JSON 형식으로 기입해야 한다.
    ```json
    {
      "email": "{이메일}",
      "authCode": "{인증 번호}"
...
- Body에 검색 장소 정보 리스트, 카테고리 리스트, 시간을 묶은 리스트와 날짜를 JSON 형식으로 기입해야 한다.
    ```json
    {
      "schedule": [
        {
...
- 호출 성공 시: 성공 메세지를 JSON 형식으로 응답한다.
    ...
    {
      "status": "OK",
      "message": "일정 저장에 성공하였습니다."
    }
    ```

- 호출 실패의 경우는 서버와의 연결 단절 밖에 존재하지 않으므로 별로의 에러 메세지는 존재하지 않는다.

<br>

### GET /api/v1/schedule?date={날짜} 
- 해당날짜 회원의 일정을 조회한다.
- 헤더에 토큰을 삽입하여 호출해야 한다.
...
- 호출 실패 시: 에러 메세지를 JSON 형식으로 응답한다.
    ...
    {
      "status": "404",
      "message": "해당 날짜의 일정 정보가 없습니다."
    }
    ```

<br>

### DELET /api/v1/schedule?date={날짜} 
- 해당날짜 회원의 일정 정보를 삭제한다.
- 헤더에 토큰을 삽입하여 호출해야 한다.
